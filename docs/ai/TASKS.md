...
- `REQ-213` | `done` | Adaptar `bodega_entrega_parcial.html` a un flujo móvil simple y vertical.
...
- `REQ-209` | `in_progress` | Ejecutar pruebas manuales en escritorio y móvil para validar bodega móvil sin regresiones; tras el pulido visual queda pendiente la validación final en dispositivo real o Chrome DevTools.
...
- `REQ-198` | `done` | Agregar exportacion CSV/XLSX en `Todas las Requisiciones`, respetando filtros activos, permisos de vista y orden operativa del listado.
- `REQ-199` | `done` | Exigir confirmacion por contrasena del bodeguero antes de ejecutar `Liquidar`, preservando valores del formulario y sin persistir ni repoblar la contrasena.
- `REQ-200` | `done` | Mostrar en el modal de detalle de `Bodega` un boton `Liquidar` cuando la requisicion ya es elegible para cierre, llevando directo al flujo de liquidacion.
