...
- `REQ-151` | `todo` | Refactor semantico del home `jefe_bodega`: reducirlo a 4 cards globales de valor real (`Pendientes por Aprobar`, `Pendientes de Procesar`, `Pendientes de Liquidar`, `Liquidadas en Prokey`) con links coherentes a `/aprobar` y `/bodega`.
- `REQ-152` | `todo` | Agregar al home `jefe_bodega` un panel SSR `Estado Global del Flujo`, con barra apilada y leyenda para `Pendiente de aprobación`, `Pendiente de proceso`, `Pendiente de liquidación`, `Finalizada` y `Rechazada`.
