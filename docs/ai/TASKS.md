...
- `REQ-179` | `todo` | Implementar el flujo SSR de confirmación en `liquidar_guardar`: leer `confirmar_diferencias` desde `form_data`, re-renderizar `liquidar.html` cuando existan diferencias sin confirmación y preservar íntegramente `liquidacion_values`, `liquidacion_meta`, `item_incompletos` e `item_validation_errors`.
- `REQ-180` | `todo` | Actualizar `templates/liquidar.html` para la confirmación no bloqueante: insertar una alerta `alert alert-warning`, enviar un hidden input `confirmar_diferencias` y cambiar el texto del botón a `Confirmar y Liquidar con Diferencias` en el segundo intento, sin trasladar lógica de validación al cliente.
