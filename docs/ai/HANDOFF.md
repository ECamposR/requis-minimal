...
- `/bodega` móvil ya inició con una primera iteración de cards móviles; `bodega_preparar.html`, `bodega_gestionar.html` y `bodega_entrega_parcial.html` ya tienen primera iteración móvil. Queda pendiente validación manual final tras el pulido visual CSS-only y el ajuste de `# Req` en la card móvil principal.
...
- Rama de trabajo actual: `feat/funcionalidades-operativas`, preparada para nuevas funciones aisladas desde `main` y el frente de exportacion de `Todas las Requisiciones`.
...
- `REQ-198` completada en `feat/funcionalidades-operativas`: `Todas las Requisiciones` ya exporta CSV/XLSX respetando filtros activos y permisos de vista, sin cambiar la consulta ni el orden del listado visible.
- `REQ-199` completada en `feat/funcionalidades-operativas`: `Liquidar` ahora exige que el usuario en sesion confirme con su contrasena antes de ejecutar el cierre; los errores re-renderizan la vista preservando cantidades/comentarios pero nunca repueblan la contrasena.
- `REQ-200` completada en `feat/funcionalidades-operativas`: el modal de detalle ahora muestra `Liquidar` junto a `Ver PDF` cuando la requisicion ya cumple condiciones operativas para cerrar, usando la misma elegibilidad de backend.
